"""
Script for preprocessing the dialog data where there are multiple interactions between Emely and the user.
...
class Dialog(BaseModel):
    # Dialog class used to store each interaction.
...
def check_emely_first_and_alternating(dialog):
    ...
    last_tag = ""
    for k in range(len(dialog.dialog)):
        tag = dialog.dialog[k][0]
        if last_tag == tag:
    ...
    if dialog.dialog[-1][0] != "e":
        dialog.dialog = dialog.dialog[:-1]
        dialog.len = len(dialog.dialog)
...
def store_data(dialog, output_path):
    """Saves the data in the desried file"""

    # Check so that the first response is from Emely.
    add_data, error_message = check_emely_first_and_alternating(dialog)
    if not add_data:
    ...
    # print(save_path)
    # Convert the dialog object to a dict.
    json_str = json.dumps(dialog.__dict__)
...
def run_data_extraction(lines, output_path, file_path):
    ...
    for k, line in enumerate(lines):
        ...
        if "episode_done" in line:
            ...
            dialog = Dialog(len=len(current_lines),
                                position=current_position,
                                dialog=current_lines,
                                emely_start=emely_start)
            #Store the data.
            store_bool, error_message = store_data(dialog, output_path)
...
def main(input_path, store_path, run_remove=False):
    """
    Goes through all files in the input path and turns the dialog to .json files.
    """
